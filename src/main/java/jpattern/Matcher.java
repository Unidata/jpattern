...
public class Matcher implements MatchResult
{
    ...
    static OutputVariable out = null;
    static OutputVariable err = null;
    static InputVariable in = null;
...

    {
        if(!(parent instanceof PE))
            throw new Failure("Matcher: illegal pattern argument:" + parent);
        setPattern(parent);
        setSubject(subject);
        setVarMap(vars);
    ...
    public Matcher setStackSize(int n)
    {
        if(n <= 0)
            throw new Failure("Matcher.setStackSize: illegal size: " + n);
        S = null;
        stackSize = n;
        return this;
    }
    ...
    // Alias
    public Matcher reset(String newsubject)
    {
        return setSubject(newsubject);
    }
    ...
    public Matcher setPattern(Pattern p)
    {
        Pat = (PE) p;
        return this;
    }
    ...
    public void setIO()
    {
        setIO(new BufferedReader(new InputStreamReader(System.in)),
            new PrintWriter(System.out),
            new PrintWriter(System.err));
    }

    public void setIO(BufferedReader stdin,
                      PrintWriter stdout,
                      PrintWriter stderr)
    {
        this.stdin = stdin;
        this.stdout = stdout;
        this.stderr = stderr;
        //Define the standard SNOBOL4 IO variables
        if(this.Vars == null)
            setVarMap(null);
        if(this.stdout != null) this.out = new OutputVariable(this.stdout);
        if(this.stderr != null) this.err = new OutputVariable(this.stderr);
        if(this.stdin != null) this.in = new InputVariable(this.stdin);
        restoreIO();
    }

    protected void restoreIO()
    {
        if(this.Vars.get("OUTPUT") == null)
            this.Vars.put("OUTPUT", out);
        if(this.Vars.get("Output") == null)
            this.Vars.put("Output", out);
        if(this.Vars.get("INPUT") == null)
            this.Vars.put("INPUT", in);
        if(this.Vars.get("Input") == null)
            this.Vars.put("Input", in);
...
//
    ...
    {
        Done, Stop, Succeed, Fail, Continue;
    }
    ...
    protected boolean XMatch()
    {
        ...
        if(Pat == null) throw new Failure("Uninitialized_Pattern");
        ...
        for(;;) {
            ...
            Debug.println(1, "Match Node=" + Node);
            ...
            case PC_Any_CH: {
                report.print(Cursor, Node);
                Debug.println(1, "any: cursor=" + Cursor);
                Debug.println(1, "any: length=" + Length);
            ...
            case PC_Any_V: {
                report.print(Cursor, Node);
                if(Vars == null)
                    throw new Failure("No VarMap context Provided: " + Node.Var);
...
//		throw new Failure("Pattern_Stack_Overflow");
                {
                    state = State.Succeed;
                    break step;
                }
            ...
            case PC_Assign_Imm: {
                ...
                Debug.println(2, "Assign_Imm: " + Node.Var + "=" + val);
                if(Vars == null)
                    throw new Failure("Attempt to write with no var map");
            ...
            case PC_BreakX_V: {
                ...
                if(Vars == null)
                    throw new Failure("No VarMap context Provided: " + Node.Var);
                if(Node.Var == null)
                    throw new Failure("Null Variable");
                String s = Vars.getString(Node.Var);
                boolean fail = true;
                while(Cursor < Length) {
                    if(Is_In(Subject.charAt(Cursor), s)) {
            ...
            //  BreakX_CH_X(BreakX extension when one char). See section on "Compound Pattern
            //  Structures". This node is the alternative that is stacked to
            //  skip past the break character and extend the break.
            case PC_BreakX_CH_X: {
                report.print(Cursor, Node);
                boolean fail = true;
                Cursor = Cursor + 1; // move cursor past last break character
                // Search for next breakx point
                while(Cursor < Length) {
                    if(Subject.charAt(Cursor) == Node.Char) {
                        fail = false;
                        break;
            ...
            case PC_EOP: {
                if(S.Stack_Base == S.Stack_Init) {
                    ...
                    Debug.println(4,"eop: " + S.prettyPrint());
                    S.Pop_Region();
                    {
                        state = State.Continue;
                        break step;
            ...
            case PC_External_X: {
                ...
                if(Externs == null)
                    throw new Failure("External.initial: no ExternalMap context Provided: " + xpname);
                ExternalPattern xp = Externs.get(xpname);
                if(xp == null)
                    throw new Failure("External.initial: no External matching name: " + xpname);
                Object[] argv = Node.Argv;
                int argc = (argv == null ? 0 : argv.length);
                if(xp.getNargs() < argc)
                    throw new Failure("External.initial: too few parameters: "
                        + xpname
                ...
                if(externalMatcher.Cursor > Length)
                    throw new Failure("External.initial: illegal cursor: " + externalMatcher.Cursor);
            ...
            case PC_External_Y: {
                ...
                ExternalPattern xp = Externs.get(xpname);
                if(xp == null)
                    throw new Failure("External.retry: no External matching name: " + xpname);
                ...
                if(externalMatcher.Cursor > Length)
                    throw new Failure("External pattern illegal cursor: " + externalMatcher.Cursor);
                Cursor = externalMatcher.Cursor;
...
// wrong: S.Stack_Ptr = Cursor - 2;
            ...
            case PC_Rpat: {
                ...
                Debug.println(3, "vars[" + Node.Var + "]=" + o);
                if(o == null) {
                    state = State.Fail;
                    break step;
                }
            ...
            default:
                throw new Failure("Logic_Error: " + Node.Pcode);
            ...
            case Done: //	 Come here if entire match succeeds
                ...
                Result.Stop = Cursor;
                Debug.println(1, "Match Succeeded");
                Debug.println(1, "Range[%d..%d]=%s",
                    ...
                    for(int e = S.Stack_Init;e <= S.Stack_Ptr;e++) {
                        if(S.Stack[e].Node == CP_Assign) {
                            ...
                            if(Node_OnM.Pcode == PC_Assign_OnM) {
                                ...
                                Debug.println(2, "Deferred Assign: %s=%s", S.Stack[e].Node, val);
                            } else
                                throw new Failure("Logic_Error");
                        }
                    }
                }
            ...
            case Fail:
                ...
                S.Pop();
                if(Cursor >= 0) {
                    Debug.println(3, "Fail: cursor->" + Cursor);
                }
...
*/
    ...
    protected boolean Is_In(char C, String Str)
    {
        return Str.indexOf(C) >= 0;
    }

    static int cursorToPtr(int Cursor)
    {
        return (Cursor & PTRMASK);
    }

    static int ptrToCursor(int ptr)
    {
        return (ptr | PTRFLAG);
    }

    static boolean isPtr(int Cursor)
    {
        return ((Cursor & PTRFLAG) == PTRFLAG);
    }
    ...
    protected final class Report
    {
        ...
        public Report(String sub, Stack Stk)
        {
            S = Stk;
            Subject = sub;
        }
        ...
        public void print(int Cursor, PE Node)
        {
            split(Cursor);
            if(Debug.level < 3) {
                Debug.println(1, "%d: \"%s^%s^%s\"; node=%s",
                    S.Region_Level,
                    prefix, range, suffix,
        ...
        void split(int Cursor)
        {
            ...
            Debug.println(1, "cursor=%s%d top=%d start=%d stop=%d prefix=|%s| range=|%s| suffix=|%s| |subject|=%d subject=|%s|",
